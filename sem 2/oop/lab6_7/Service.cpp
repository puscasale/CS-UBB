...
void Service::add(string name, string description, string type, int time){
    ...
    rep.store(a);
    undo.push_back(make_unique<AddUndo>(rep));
}

void Service::del(int poz){
    Activity a = rep.get_activity(poz);
    rep.remove(poz);
    undo.push_back(make_unique<DeleteUndo>(rep, a, poz));
}


void Service::modify(string new_name, string new_description, string new_type, int new_time, int index){
    valid.validateActivity(new_name, new_description, new_type, new_time);
    const Activity& a{new_name, new_description, new_type, new_time};
    Activity b = rep.get_activity(index);
    rep.modify(index, a);
    undo.push_back(make_unique<ModifyUndo>(rep, b, index));
}
...
const Activity &Service::get_activity(int index) {
    return rep.get_activity(index);
}
