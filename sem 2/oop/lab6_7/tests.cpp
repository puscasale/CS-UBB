...
void test_store_p() {
    ...
    rep.store(a);
    assert(rep.get_all().size() == 1);
    rep.modify(0, Activity{"a","a","a",8});
    assert(rep.get_all()[0].get_time() == 8);
    rep.remove(0);
    assert(rep.get_all().empty());
    rep.store(a);
    assert(rep.get_activity(0).get_time() == 22);
...
void test_get_activity() {
    ...
    Activity a( "a", "ab", "ac", 22);
    rep.store(a);
    Activity rez = rep.get_activity(0);
...
void test_modify_srv() {
    ...
    Service srv{rep, valid};
    srv.add("a", "s", "b", 22);
    srv.modify("w", "e", "r", 4, 0);
    assert(srv.get_activity(0).get_time() == 4);
}
...
void test_file() {
    RepoInFile repo(R"(C:\Users\Ale\Desktop\oop\lab6_7\test_file.txt)");
    ...
    repo.modify(1, Activity("a","r","e",6));
